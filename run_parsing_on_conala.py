...
from parser import parse_sentence, connect_tagger
...
def parse_conala(args):
    ...
    a_file = open(args.data)
    file_contents = a_file.readlines()
    conala = file_contents
    ...
    with open(args.outdir + '/failed_queries.txt', 'w') as f_out, \
        ...
        if args.n:
            data_gen = tqdm(enumerate(conala[:args.n]))
        else:
            data_gen = tqdm(enumerate(conala))
        for i, q in data_gen:
            all_q_count += 1
            q = ' '.join(q.split()[:-1])
            q = q.lower()
            ...
            data_gen.set_description("success rate: {:.2f}".format(
                float(parsed_q_count) / all_q_count), refresh=True)
    sys.stdout.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument("-outdir", dest="outdir", type=str,
                        help="Directory to create output files", required=True)
    parser.add_argument("-time_limit", dest="time_limit",
                        type=int, help="Timeout for parsing")
